...
app.get("/api/appointments", (req, res) => {
  const appointments = getAllProviders().flatMap((provider) =>
    provider
      ...
      .map(({ id, ...a }) => ({
        ...a,
        externalReference: encodeRef(provider.system, id),
      })),
  );
...
app.post("/api/appointments/:externalReference/reschedule", (req, res) => {
  const { system: apptSystem, id: apptId } = decodeRef(
  ...
  console.log(
    `[Business Logic] Rescheduling appointment ${apptSystem}:${apptId} to ${timeslot.dateTime}`,
  );
  provider.reschedule(apptId, slotId, timeslot.dateTime);
  console.log(
    `[Business Logic] Request sent to care unit for appointment ${apptSystem}:${apptId}`,
  );
